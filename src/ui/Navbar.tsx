...
function Navbar() {
  ...
  const saveReturn = () => setAuthReturnTo(location.pathname + location.search);

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center bg-semiDarkBlue px-4 py-3 lg:sticky lg:top-0 lg:z-[9999] lg:h-screen lg:w-[6rem] lg:flex-shrink-0 lg:flex-col lg:py-8">
      <img
      ...
      <div className="flex w-full items-center justify-between lg:w-auto lg:flex-col lg:gap-[3.75rem]">
        {NAV_ITEMS.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
          ...
          >
            <NavIcon icon={item.icon} className="h-7 w-7 lg:h-5 lg:w-5" />
      ...
      {isAuthenticated ? (
        <div className="hidden items-start gap-8 lg:mt-10 lg:flex lg:flex-col">
          <NavLink
            to="/profile"
            className="group/item relative flex items-center justify-center text-grayishBlue transition-colors duration-200 hover:text-white lg:w-full"
          >
          ...
          <button
            onClick={() => logout()}
            disabled={isPending}
            className="group/item relative flex items-center justify-center text-grayishBlue transition-colors duration-200 hover:text-red lg:w-full"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
